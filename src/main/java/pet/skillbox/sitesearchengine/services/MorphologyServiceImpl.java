...
public class MorphologyServiceImpl {
    ...
    public Map<String, Integer> getNormalFormsList(String text) throws IOException {
        ...
        String[] words = text.replaceAll("[^[а-яА-Я ]]", " ").toLowerCase().split("\\s+");

        Arrays.stream(words).forEach(word -> {
            String[] tmp;
            if (!word.trim().isEmpty()) {
                List<String> normalWordL = luceneMorphology.getNormalForms(word);
                ...
                tmp = luceneMorphology.getMorphInfo(word).get(0).split("\\s+");
